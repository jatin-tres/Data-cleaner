...
if 'Transaction Hash' in df.columns:
    try:
        ...
        # Convert to string, replacing NaN-assigned 0s with empty string
...
with tab2:
    ...
    if selected_currency:
        ...
        # Display includes new grouping columns
        display_cols = [
            'Timestamp', 
...
with tab4:
    ...
    st.markdown("Displays the **cumulative balance (net asset holdings)** for a selected token, sorted by **Timestamp**. Includes a **`Balance Status`** warning and **`Group ID`**.")
...
with tab5:
    ...
    with st.expander("Report 5: Monthly Transaction Volume", expanded=True):
        ...
        if 'Timestamp' in df.columns and 'Total Fiat Amount ($)' in df.columns:
            try:
                ...
                # FIX: replaced use_container_width=True with width='stretch'
                st.altair_chart(line_count + bar_value, width='stretch')
    ...
    with st.expander("Report 6: Top 10 Counterparties (By Count)"):
        ...
        if counterparty_col:
            ...
            # FIX: replaced use_container_width=True with width='stretch'
            st.dataframe(top_counterparties, width='stretch')
            ...
            # FIX: replaced use_container_width=True with width='stretch'
            st.altair_chart(chart_cp, width='stretch')
        else:
